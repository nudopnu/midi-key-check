...
import { GrandStaff } from './vexflow.service';
...
export class VexflowService {
  ...
  drawMidis(element: HTMLDivElement, midis: number[], upperStave = true) {
    if (!upperStave) {
    ...
    const grandStaff = new GrandStaff(element, new Key("Gb", "Ionian"));
    grandStaff.drawNotes(notes, upperStave);
  }
